...
import { Currency, ETHER, Token, currencyEquals } from '@smartdev1990/quick-sdk1';
...
export function useTokens(tokenAddresses: string[]): Token[] | undefined | null {
	const { chainId } = useActiveWeb3React();
	const tokens = useAllTokens();
	...
	return useMemo(() => {
		const existingTokens = [...new Set(tokenAddresses)]
			...
			.map((x) => tokens[x])
			.filter((x) => !!x);
		return validatedUnknownTokenAddresses.reduce<Token[]>((memo, tokenAddress, index) => {
