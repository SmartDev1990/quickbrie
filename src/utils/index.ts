...
export const getEthPrice: () => Promise<number[]> = async () => {
	...
	try {
		...
		const oneDayBackPrice = Number(resultOneDay?.data?.bundles[0]?.ethPrice ?? 0);
...
export const getTokenInfo = async (ethPrice: number, ethPriceOld: number, address: string) => {
	const utcCurrentTime = dayjs();
	const utcOneDayBack = utcCurrentTime.subtract(1, 'day').unix();
	...
	try {
		...
		const oneWeekData = oneWeekResult?.data?.tokens.reduce((obj: any, cur: any) => {
			return { ...obj, [cur.id]: cur };
		}, {});
		...
		const bulkResults = await Promise.all(
			current &&
				...
				current?.data?.tokens?.map(async (token: any) => {
					...
					const oneWeekVolumeUSD = data.tradeVolumeUSD - (oneWeekHistory?.tradeVolumeUSD ?? 0);

					const currentLiquidityUSD = data?.totalLiquidity * ethPrice * data?.derivedETH;
					const oldLiquidityUSD =
						(oneDayHistory?.totalLiquidity ?? 0) * ethPriceOld * (oneDayHistory?.derivedETH ?? 0);
...
export const getTopTokens = async (ethPrice: number, ethPriceOld: number, count = 500) => {
	const utcCurrentTime = dayjs();
	const utcOneDayBack = utcCurrentTime.subtract(1, 'day').unix();
	const utcTwoDaysBack = utcCurrentTime.subtract(2, 'day').unix();
	const oneDayBlock = await getBlockFromTimestamp(utcOneDayBack);
...
export const getTokenPairs = async (tokenAddress: string, tokenAddress1: string) => {
	try {
...
export const getPairAddress = async (token0Address: string, token1Address: string) => {
...
export const getSwapTransactions = async (pairId: string, startTime?: number) => {
	let allFound = false;
	let swapTx: any[] = [];
	...
	return swapTx
		.filter((item, ind, self) => ind === self.findIndex((item1) => item1.transaction.id === item.transaction.id))
		.reverse();
};

export const getTokenChartData = async (tokenAddress: string, startTime: number) => {
	let data: any[] = [];
	const utcEndTime = dayjs.utc();
	try {
		let allFound = false;
...
export const getPairChartData = async (pairAddress: string, startTime: number) => {
	let data: any[] = [];
	const utcEndTime = dayjs.utc();
	try {
		let allFound = false;
	...
	try {
		...
		for (const row in result) {
			...
			if (timestamp) {
				values.push({
					timestamp,
					rate: pairTokenReversed ? Number(result[row]?.token0Price) : Number(result[row]?.token1Price),
				});
			}
		}
		return values;
...
export const getBulkPairData: (pairList: any, ethPrice: any) => Promise<any[] | undefined> = async (
	pairList: any,
	ethPrice: any,
) => {
...
const parseData = (data: any, oneDayData: any, twoDayData: any, oneWeekData: any, ethPrice: any, oneDayBlock: any) => {
	// get volume changes
	const [oneDayVolumeUSD, volumeChangeUSD] = get2DayPercentChange(
...
export async function getGlobalData(ethPrice: number, oldEthPrice: number): Promise<any> {
...
export function isTokensOnList(defaultTokens: TokenAddressMap, currencies: (Currency | undefined)[]): boolean[] {
...
export function getEtherscanLink(
	...
	const prefix = 'https://' + (chainId === 64668 ? 'TESTNET.' : '') + 'polygonscan.com';
...
export function formatDateFromTimeStamp(timestamp: number, format: string, addedDay = 0) {
	return dayjs
		.unix(timestamp)
		.add(addedDay, 'day')
		.utc()
...
export function formatNumber(unformatted: number | string | undefined, showDigits = 2) {
...
export function getChartStartTime(durationIndex: number) {
	...
	const startTime =
		utcEndTime
			.subtract(months, durationIndex === GlobalConst.analyticChart.ONE_YEAR_CHART ? 'year' : 'month')
			.endOf('day')
			.unix() - 1;
	return startTime;
}
...
export function getTokenAPRSyrup(syrup: SyrupInfo) {
	return syrup.valueOfTotalStakedAmountInUSDC && syrup.valueOfTotalStakedAmountInUSDC > 0
		? ((syrup.rewards ?? 0) / syrup.valueOfTotalStakedAmountInUSDC) * getDaysCurrentYear() * 100
		: 0;
}
...
export function useLairDQUICKAPY(isNew: boolean, lair?: LairInfo) {
	...
	const dQUICKAPR =
		(((Number(lair.oneDayVol) * GlobalConst.utils.DQUICKFEE * GlobalConst.utils.DQUICKAPR_MULTIPLIER) /
...
export function getWalletKeys(connector: AbstractConnector | undefined): string[] {
...
export function getRewardRate(rate?: TokenAmount, rewardToken?: Token) {
	...
	return `${rate.toFixed(2, { groupSeparator: ',' }).replace(/[.,]00$/, '')} ${rewardToken.symbol}  / day`;
}
...
export function getStakedAmountStakingInfo(
	...
	const valueOfTotalStakedAmountInUSDC = USDPrice?.quote(valueOfTotalStakedAmountInBaseToken);

	const valueOfMyStakedAmountInUSDC = USDPrice?.quote(valueOfMyStakedAmountInBaseToken);
	...
	const valueOfUnstakedAmountInUSDC = USDPrice?.quote(valueOfUnstakedAmountInBaseToken);
	return {
		...stakedAmounts,
...
export function formatTokenAmount(amount?: TokenAmount | CurrencyAmount, digits = 3) {
	if (!amount) return '-';
...
export function isSupportedNetwork(ethereum: any) {
	return Number(ethereum.chainId) === 32520;
}
...
export function getPartialTokenAmount(percent: number, amount?: TokenAmount | CurrencyAmount) {
	if (!amount) return '0';
...
export function initTokenAmountFromCallResult(token: Token, callState?: CallState) {
	if (!callState || !callState.result || !callState.result[0]) return;
	return new TokenAmount(token, JSBI.BigInt(callState.result[0]));
}

export function getFarmLPToken(info: StakingInfo | DualStakingInfo | StakingBasic | DualStakingBasic) {
	const lp = info.lp;
	const dummyPair = new Pair(new TokenAmount(info.tokens[0], '0'), new TokenAmount(info.tokens[1], '0'));
	if (lp && lp !== '') return new Token(32520, lp, 18, 'RLP', 'Riceswap LP');
