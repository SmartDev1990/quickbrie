import { ChainId, Currency, currencyEquals, JSBI, Price, WETH, Token } from '@smartdev1990/quick-sdk1';
...
export default function useUSDCPrice(currency?: Currency): Price | undefined {
	...
	const tokenPairs: [Currency | undefined, Currency | undefined][] = useMemo(
		() => [
			[
				chainId && wrapped && currencyEquals(WETH[chainId], wrapped) ? undefined : wrapped,
			...
			[wrapped?.equals(usdcToken) ? undefined : wrapped, chainId === ChainId.MAINNET ? usdcToken : undefined],
			[wrapped?.equals(usdtToken) ? undefined : wrapped, chainId === ChainId.MAINNET ? usdtToken : undefined],
			[chainId ? WETH[chainId] : undefined, chainId === ChainId.MAINNET ? usdcToken : undefined],
			[
				chainId === ChainId.MAINNET ? oldQuickToken : undefined,
				chainId === ChainId.MAINNET ? usdcToken : undefined,
			],
		],
		[chainId, wrapped, oldQuickToken, usdcToken, usdtToken],
	);
	const [
...
export function useUSDCPrices(currencies: Currency[]): (Price | undefined)[] {
	...
	wrappedCurrencies.forEach((wrapped, ind) => {
		tokenPairs.push([
			chainId && wrapped && currencyEquals(WETH[chainId], wrapped) ? undefined : currencies[ind],
			chainId ? WETH[chainId] : undefined,
		]);
		tokenPairs.push([
			wrapped?.equals(oldQuickToken) ? undefined : wrapped,
...
export function useUSDCPricesToken(tokens: Token[]) {
	...
	const quickPrice = Number(quickUsdcPair?.priceOf(oldQuickToken)?.toSignificant(6) ?? 0);
	const newQuickPrice = Number(newQuickUsdcPair?.priceOf(newQuickToken)?.toSignificant(6) ?? 0);
