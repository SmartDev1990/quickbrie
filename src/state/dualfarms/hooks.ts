...
export class WrappedDualFarmInfo implements DualStakingBasic {
	...
	constructor(
		...
		this.tokens = [
			getTokenFromAddress(stakingInfo.tokens[0], chainId, tokenAddressMap, dualFarmTokens),
			getTokenFromAddress(stakingInfo.tokens[1], chainId, tokenAddressMap, dualFarmTokens),
		];
...
const dualFarmCache: WeakMap<DualFarmListInfo, DualFarmInfoAddressMap> | null =
	typeof WeakMap !== 'undefined' ? new WeakMap<DualFarmListInfo, DualFarmInfoAddressMap>() : null;
...
export function useDualFarmList(url: string | undefined): DualFarmInfoAddressMap {
	const dualFarms = useSelector<AppState, AppState['dualFarms']['byUrl']>((state) => state.dualFarms.byUrl);
