...
const initialState: DualFarmsListsState = {
	...
	byUrl: {
		...DEFAULT_LIST_OF_LISTS.reduce<Mutable<DualFarmsListsState['byUrl']>>((memo, listUrl) => {
...
export default createReducer(initialState, (builder) =>
	builder
		.addCase(fetchDualFarmList.pending, (state, { payload: { requestId, url } }) => {
			state.byUrl[url] = {
				...state.byUrl[url],
		...
		.addCase(fetchDualFarmList.fulfilled, (state, { payload: { requestId, dualFarmList, url } }) => {
			const current = state.byUrl[url]?.current;
			const loadingRequestId = state.byUrl[url]?.loadingRequestId;

			// no-op if update does nothing
			if (current) {
				const upgradeType = getVersionUpgrade(current.version, dualFarmList.version);
		...
		.addCase(fetchDualFarmList.rejected, (state, { payload: { url, requestId, errorMessage } }) => {
