...
export class WrappedSyrupInfo implements SyrupBasic {
	...
	constructor(syrupInfo: SyrupRaw, tokenAddressMap: TokenAddressMap, syrupTokens: Token[], chainId: ChainId) {
...
const syrupCache: WeakMap<SyrupListInfo, SyrupInfoAddressMap> | null =
	typeof WeakMap !== 'undefined' ? new WeakMap<SyrupListInfo, SyrupInfoAddressMap>() : null;
...
export function listToSyrupMap(
	...
	const map = list.active.concat(list.closed).reduce<SyrupInfoAddressMap>(
		(syrupInfoMap, syrup) => {
			...
			if (syrupInfoMap[wrappedSyrupInfo.chainId][wrappedSyrupInfo.stakingRewardAddress] !== undefined)
...
export function useSyrupList(url: string | undefined): SyrupInfoAddressMap {
	const syrups = useSelector<AppState, AppState['syrups']['byUrl']>((state) => state.syrups.byUrl);
	...
	const syrupTokenAddresses =
		current && tokenMap
			? current.active
					...
					.filter(
						(address, ind, self) =>
							self.findIndex((addr) => address.toLowerCase() === addr.toLowerCase()) === ind,
					)
			: [];
	const syrupTokens = useTokens(syrupTokenAddresses);
	return useMemo(() => {
		if (!current || !tokenMap || syrupTokenAddresses.length !== syrupTokens?.length) return EMPTY_LIST;
		try {
