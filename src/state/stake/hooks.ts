import { CurrencyAmount, JSBI, Token, TokenAmount, Pair, ChainId } from '@smartdev1990/quick-sdk1';
...
import { CallState, NEVER_RELOAD, useMultipleContractSingleData, useSingleCallResult } from 'state/multicall/hooks';
...
import { SyrupInfo, LairInfo, StakingInfo, DualStakingInfo, StakingBasic, DualStakingBasic } from 'types';
...
export function useTotalRewardsDistributed(chainId: ChainId): number {
	...
	const syrupTokenPairs = usePairs(
		syrupRewardsInfo.map((item) => [unwrappedToken(item.token), unwrappedToken(item.baseToken)]),
	);
	...
	const syrupRewardsUSD = syrupRewardsInfo.reduce((total, item, index) => {
		...
		const priceOfRewardTokenInUSD =
			Number(tokenPairPrice?.toSignificant()) * Number(usdPriceBaseToken?.toSignificant());
	...
	const dualStakingRewardsUSD = dualStakingRewardsInfo.reduce(
		(total, item, index) => total + item.rateA * rewardTokenAPrices[index] + item.rateB * rewardTokenBPrices[index],
		0,
	);
...
export function useUSDRewardsandFees(
	...
	const rewardsAddresses = useMemo(() => rewardsInfos.map(({ stakingRewardAddress }) => stakingRewardAddress), [
		rewardsInfos,
	]);
	...
	const rewardPairs = useMemo(() => rewardsInfos.map(({ pair }) => pair), [rewardsInfos]);
	...
	if (isLPFarm) {
		rewardsUSD = stakingRewardsInfo.reduce(
			(total, item, index) => total + item.rate * stakingRewardTokenPrices[index],
			0,
		);
	} else {
	...
	const stakingFees = bulkPairData
		? rewardPairs.reduce((total, pair, index) => {
				...
				if (oneYearFeeAPY) {
					const totalSupply = web3.utils.toWei(pairs[pair]?.totalSupply, 'ether');
...
export function useFilteredSyrupInfo(
	...
	const info = useMemo(
		...
		[tokenToFilterBy, startIndex, endIndex, filter, currentTimestamp, allSyrups],
	);
	...
	const stakingTokenPairs = usePairs(
		info.map((item) => [unwrappedToken(item.token), unwrappedToken(item.baseToken)]),
	);
	...
	return useMemo(() => {
		...
		filter && filter.isStaked ? syrupInfo.stakedAmount && syrupInfo.stakedAmount.greaterThan('0') : true,
	);
}
...
export const getBulkPairData = async (pairList: any) => {
	...
	try {
		...
		const oneDayData = oneDayResult?.data?.pairs.reduce((obj: any, cur: any, i: any) => {
...
function parseData(data: any, oneDayData: any) {
	// get volume changes
	const oneDayVolumeUSD = get2DayPercentChange(data?.volumeUSD, oneDayData?.volumeUSD ? oneDayData.volumeUSD : 0);
	return {
		id: data.id,
		token0: data.token0,
		token1: data.token1,
...
export function getStakingFees(
	...
	if (pairs !== undefined) {
		...
		if (oneYearFeeAPY && balanceResult && totalSupplyResult) {
			const totalSupply = web3.utils.toWei(pairs[stakingInfo.pair]?.totalSupply, 'ether');
			...
			oneYearFeeAPY = getOneYearFee(oneYearFeeAPY, pairs[stakingInfo.pair]?.reserveUSD);
		}
	}
...
export function useDualStakingInfo(
	...
	const info = useMemo(
		...
		[chainId, pairToFilterBy, startIndex, endIndex, filter, dualStakingRewardsInfo],
	);
	...
	return useMemo(() => {
		...
		return rewardsAddresses.reduce<DualStakingInfo[]>((memo, rewardsAddress, index) => {
			...
			if (
				...
				const totalStakedAmount = initTokenAmountFromCallResult(lpFarmToken, totalSupplyState);
				...
				const totalRewardRateB = new TokenAmount(dummyToken, JSBI.BigInt(stakingInfo.ended ? 0 : rateB));
				//const pair = info[index].pair.toLowerCase();
				//const fees = (pairData && pairData[pair] ? pairData[pair].oneDayVolumeUSD * 0.0025: 0);
				...
				const { oneDayFee, accountFee } = getStakingFees(stakingInfo, balanceState, totalSupplyState);
				...
				if (totalSupply && stakingTokenPair && baseTokens[index] && totalStakedAmount) {
					...
					valueOfTotalStakedAmountInBaseToken = new TokenAmount(
						...
						JSBI.divide(
							JSBI.multiply(
								JSBI.multiply(totalStakedAmount.raw, stakingTokenPair.reserveOf(baseTokens[index]).raw),
				...
				const perMonthReturnInRewards =
					((stakingInfo.rateA * rewardTokenAPrice + stakingInfo.rateB * rewardTokenBPrice) *
		...
		filter && filter.isStaked ? stakingInfo.stakedAmount && stakingInfo.stakedAmount.greaterThan('0') : true,
	);
}
...
function useLairInfo(
	...
	return useMemo(() => {
		return {
			...
			dQUICKtoQUICK: new TokenAmount(quickToken, JSBI.BigInt(dQuickToQuick?.result?.[0] ?? 0)),
			QUICKtodQUICK: new TokenAmount(dQuickToken, JSBI.BigInt(quickToDQuick?.result?.[0] ?? 0)),
			dQUICKBalance: new TokenAmount(dQuickToken, JSBI.BigInt(dQuickBalance?.result?.[0] ?? 0)),
			QUICKBalance: new TokenAmount(quickToken, JSBI.BigInt(quickBalance?.result?.[0] ?? 0)),
			totalQuickBalance: new TokenAmount(quickToken, JSBI.BigInt(lairsQuickBalance?.result?.[0] ?? 0)),
			dQuickTotalSupply: new TokenAmount(dQuickToken, JSBI.BigInt(_dQuickTotalSupply?.result?.[0] ?? 0)),
			oneDayVol: oneDayVol,
		};
	}, [
		lairAddress,
...
export function useStakingInfo(
	...
	const totalSupplys = useTotalSupplys(
		info.map((item) => {
			...
			return lp && lp !== '' ? new Token(32520, lp, 18, 'RLP', 'Riceswap LP') : dummyPair.liquidityToken;
		}),
	);
	const stakingPairs = usePairs(info.map((item) => item.tokens));
	...
	return useMemo(() => {
		...
		return rewardsAddresses.reduce<StakingInfo[]>((memo, rewardsAddress, index) => {
			...
			if (
				...
				const perMonthReturnInRewards =
					(Number(stakingInfo.rate) * rewardTokenPrice * (getDaysCurrentYear() / 12)) /
					Number(valueOfTotalStakedAmountInUSDC?.toExact());
...
export function useOldStakingInfo(
	...
	}, [balances, chainId, earnedAmounts, info, rewardsAddresses, totalSupplies, stakingPairs]).filter((stakingInfo) =>
		filter && filter.isStaked ? stakingInfo.stakedAmount && stakingInfo.stakedAmount.greaterThan('0') : true,
	);
}
