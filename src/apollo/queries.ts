...
export const TOKEN_DATA1: any = (tokenAddress: string, tokenAddress1: string) => {
	const queryString = `
    query tokens {
...
export const GLOBAL_CHART = gql`
	query uniswapDayDatas($startTime: Int!, $skip: Int!) {
		uniswapDayDatas(first: 500, skip: $skip, where: { date_gt: $startTime }, orderBy: date, orderDirection: asc) {
			id
			date
			totalVolumeUSD
			dailyVolumeUSD
...
export const SWAP_TRANSACTIONS = gql`
	query($allPairs: [Bytes]!, $lastTime: Int!) {
		swaps(first: 1000, where: { pair_in: $allPairs, timestamp_gte: $lastTime }, orderBy: timestamp) {
			transaction {
				id
				timestamp
			}
