...
import { ButtonGroup, Button, Box, Grid, useMediaQuery } from '@material-ui/core';
	...
	return (
		<>
			<Box className='buttonGroup'>
				<ButtonGroup>
					<Button className={tabIndex === SWAP_TAB ? 'active' : ''} onClick={() => setTabIndex(SWAP_TAB)}>
						{t('swap')}
					</Button>
					<Button
			...
			<Box className='swapContainer'>
				<Grid container spacing={mobileWindowSize ? 0 : 8} alignItems='center'>
					...
					<Grid item sm={12} md={6} className='swapInfo'>
						<h4>{tabIndex === SWAP_TAB ? t('swapSectionShortDesc') : t('liquiditySectionShortDesc')}</h4>
						<p style={{ marginTop: '20px' }}>
							{tabIndex === SWAP_TAB ? t('swapSectionLongDesc') : t('liquiditySectionLongDesc')}
						</p>
					</Grid>
				</Grid>
			</Box>
