...
const AnalyticsPairChart: React.FC<{ pairData: any }> = ({ pairData }) => {
	...
	const fees =
		pairData && (pairData.oneDayVolumeUSD || pairData.oneDayVolumeUSD === 0)
			? usingUtVolume
				? (Number(pairData.oneDayVolumeUntracked) * GlobalConst.utils.FEEPERCENT).toLocaleString()
				: (Number(pairData.oneDayVolumeUSD) * GlobalConst.utils.FEEPERCENT).toLocaleString()
			: '-';
						...
						{currentPercent && currentData ? (
							<>
								<Box className='flex items-center'>
									...
									<Box className={`priceChangeWrapper ${currentPercentClass}`} ml={1}>
