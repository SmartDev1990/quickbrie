...
const DragonsLair: React.FC<{ isNew: boolean }> = ({ isNew }) => {
	...
	return (
		<Box position='relative' zIndex={3}>
			{openStakeModal && <StakeQuickModal open={openStakeModal} onClose={() => setOpenStakeModal(false)} />}
			{openUnstakeModal && (
				<UnstakeQuickModal open={openUnstakeModal} onClose={() => setOpenUnstakeModal(false)} isNew={isNew} />
			)}
			<Box display='flex'>
			...
			<Box className='dragonLairRow'>
				<small>{t('tvl')}</small>
				<small>${(Number(lairInfoToUse.totalQuickBalance.toExact()) * quickPrice).toLocaleString()}</small>
			</Box>
			<Box className='dragonLairRow'>
				<small>{t('apy')}</small>
			...
			<Box className='quickTodQuick border-secondary1'>
				...
				<small style={{ margin: '0 8px' }}>{isQUICKRate ? 1 : dQUICKtoQUICK.toLocaleString()} QUICK =</small>
				<CurrencyLogo currency={quickToken} />
				<small style={{ margin: '0 8px' }}>{isQUICKRate ? QUICKtodQUICK.toLocaleString() : 1} dQUICK</small>
				<PriceExchangeIcon className='cursor-pointer' onClick={() => setIsQUICKRate(!isQUICKRate)} />
			</Box>
			{!isNew && (
				<Box className='stakeButton bg-primary' onClick={() => setOpenStakeModal(true)}>
					<small>{t('stake')}</small>
				</Box>
			)}
			<Box className='stakeButton bg-transparent' onClick={() => setOpenUnstakeModal(true)}>
