	...
	const liquidityWeeks = useMemo(() => {
		if (globalChartData) {
			...
			globalChartData.week.forEach((value: any, ind: number) => {
				...
				const monthLastDate =
					ind > 0 ? formatDateFromTimeStamp(Number(globalChartData.week[ind - 1].date), 'MMM') : '';
		...
		if (selectedVolumeIndex > -1) {
			if (volumeIndex === DAY_VOLUME) {
				...
				const weekStart = formatDateFromTimeStamp(
					Number(globalChartData.week[Math.max(0, selectedVolumeIndex - 1)].date),
					'MMM DD, YYYY',
									...
									{formatCompact(
										selectedVolumeIndex > -1
											? volumeIndex === DAY_VOLUME
												...
												: globalChartData.week[selectedVolumeIndex].weeklyVolumeUSD
											: volumeIndex === DAY_VOLUME
											? globalData.oneDayVolumeUSD
											: globalData.oneWeekVolume,
									)}
								</h5>
								<Box ml={1} height={23} px={1} borderRadius={40} className={volumePercentClass}>
									<span>
