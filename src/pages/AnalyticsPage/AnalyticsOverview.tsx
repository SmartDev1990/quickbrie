		...
		<Box width='100%' mb={3}>
			...
			<Box mt={4}>
				<Box className='panel flex flex-wrap'>
					{globalData ? <AnalyticsInfo data={globalData} /> : <Skeleton width='100%' height={20} />}
				</Box>
			</Box>
			<Box mt={4}>
				<Box className='flex justify-between items-center'>
					...
					<Box className='headingWrapper cursor-pointer' onClick={() => history.push(`/analytics/tokens`)}>
			...
			<Box mt={4}>
				<Box className='flex items-center justify-between'>
					...
					<Box className='headingWrapper cursor-pointer' onClick={() => history.push(`/analytics/pairs`)}>
						<p className='weight-600'>{t('seeAll')}</p>
						<ArrowForwardIos />
					</Box>
				</Box>
