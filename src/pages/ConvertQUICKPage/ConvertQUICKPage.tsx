...
const ConvertQUICKPage: React.FC = () => {
	...
	const [approval, approveCallback] = useApproveCallback(parsedAmount, quickConvertContract?.address);
	...
	const isInsufficientQUICK = Number(quickAmount) > Number(quickBalance?.toExact() ?? 0);
		...
		<Box width='100%' maxWidth={488} id='convertQUICKPage'>
			...
			<Box className='convertQUICKWrapper'>
				...
				<Box className='conversionRate'>
					<span>
						{t('conversionRate')}: 1 QUICK(OLD) = {GlobalConst.utils.QUICK_CONVERSION_RATE} QUICK(NEW)
					</span>
				</Box>
				<Box mt={4} mb={2}>
					<small className='text-secondary'>
					...
					<Box className={`currencyInput${isInsufficientQUICK ? ' errorInput' : ''}`}>
						<NumericalInput
							placeholder='0.00'
							value={quickAmount}
							fontSize={18}
							onUserInput={(value) => {
								...
								setQUICKV2Amount(
									(Number(fixedVal) * GlobalConst.utils.QUICK_CONVERSION_RATE).toLocaleString(
										'fullwide',
										{
											useGrouping: false,
						...
						<Box
							...
							onClick={() => {
								if (quickBalance) {
									...
									setQUICKV2Amount(
										(
											Number(quickBalance.toExact()) * GlobalConst.utils.QUICK_CONVERSION_RATE
										).toString(),
									);
								}
							}}
					...
					{isInsufficientQUICK && (
						<small className='text-error'>{t('insufficientBalance', { symbol: 'QUICK' })}</small>
					)}
				</Box>
			...
			{showConfirm && (
				<TransactionConfirmationModal
					...
					content={() =>
						txError ? (
							<TransactionErrorContent onDismiss={handleDismissConfirmation} message={txError} />
						) : (
							<ConfirmationModalContent
