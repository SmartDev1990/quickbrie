...
const FarmRewards: React.FC<{ farmIndex: number; bulkPairs: any }> = ({ farmIndex, bulkPairs }) => {
	...
	const farmData = useUSDRewardsandFees(farmIndex === GlobalConst.farmIndex.LPFARM_INDEX, bulkPairs, defaultChainId);
	...
	const getRewardsSection = (isLPFarm: boolean) => (
		<>
			...
			<Box width={isMobile ? 1 : isLPFarm ? 1 / 3 : 1 / 2} p={1.5} textAlign='center'>
				<Box mb={1}>
					<span className='text-secondary'>{t('fees24h')}</span>
				</Box>
				{farmData.stakingFees ? (
					<h6 className='weight-600'>${farmData.stakingFees.toLocaleString()}</h6>
				) : (
					<Skeleton width='100%' height='28px' />
				)}
			</Box>
	...
	return (
		<Box className='farmRewards'>
			{farmIndex === GlobalConst.farmIndex.LPFARM_INDEX && (
				<>
					<Box width={isMobile ? 1 : 1 / 3} py={1.5} className='border-right text-center'>
						<Box mb={1}>
			...
			{farmIndex === GlobalConst.farmIndex.DUALFARM_INDEX && getRewardsSection(false)}
		</Box>
	);
};
