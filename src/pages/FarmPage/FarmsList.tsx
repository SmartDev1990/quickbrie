...
const FarmsList: React.FC<FarmsListProps> = ({ bulkPairs, farmIndex }) => {
	...
	const addedLPStakingInfos = useStakingInfo(
		chainIdOrDefault,
		null,
		farmIndex === GlobalConst.farmIndex.DUALFARM_INDEX || isEndedFarm ? 0 : undefined,
		farmIndex === GlobalConst.farmIndex.DUALFARM_INDEX || isEndedFarm ? 0 : undefined,
	...
	const sortByRewardLP = useCallback(
		(a: StakingInfo, b: StakingInfo) => {
			return (
				(getExactTokenAmount(a.totalRewardRate) > getExactTokenAmount(b.totalRewardRate) ? -1 : 1) * sortIndex
			);
		},
		[sortIndex],
	);
	...
	const sortByAPY = useCallback(
		(a: CommonStakingInfo, b: CommonStakingInfo) => {
			...
			const aAPYwithFee = getAPYWithFee(a.perMonthReturnInRewards ?? 0, aYearFee);
			const bAPYwithFee = getAPYWithFee(b.perMonthReturnInRewards ?? 0, bYearFee);
	...
	const addedStakingInfos = useMemo(
		() => (farmIndex === GlobalConst.farmIndex.DUALFARM_INDEX ? sortedStakingDualInfos : sortedLPStakingInfos),
	...
	const stakingInfos = useMemo(() => {
		return sortedLPStakingInfos
			? sortedLPStakingInfos.slice(0, getPageItemsToLoad(pageIndex, LOADFARM_COUNT))
			: null;
	}, [sortedLPStakingInfos, pageIndex]);

	const stakingDualInfos = useMemo(() => {
		return sortedStakingDualInfos
			? sortedStakingDualInfos.slice(0, getPageItemsToLoad(pageIndex, LOADFARM_COUNT))
			: null;
		...
		<>
			<Box className='farmListHeader'>
				<Box>
					<h5>{t('earndQUICK')}</h5>
					<small>
						{t(farmIndex === GlobalConst.farmIndex.LPFARM_INDEX ? 'stakeMessageLP' : 'stakeMessageDual')}
					</small>
				</Box>
				<Box className='flex flex-wrap'>
					<Box className='flex justify-between' width={returnFullWidthMobile(isMobile)}>
					...
					<Box width={returnFullWidthMobile(isMobile)} className='flex flex-wrap items-center'>
						<Box mr={2}>
						...
						{isMobile ? (
							<>
								...
								<Box mt={2} width={1} className='flex items-center'>
									...
									<ToggleSwitch toggled={sortDesc} onToggle={() => setSortDesc(!sortDesc)} />
								</Box>
							</>
						) : (
							renderStakedOnly()
					...
					{sortByDesktopItems.map((item) => (
						...
						>
							...
							<Box display='flex' ml={0.5}>
								{sortBy === item.index && sortDesc ? <ArrowDown size={20} /> : <ArrowUp size={20} />}
							</Box>
						</Box>
					))}
				</Box>
			...
			{farmIndex === GlobalConst.farmIndex.LPFARM_INDEX &&
				...
				stakingInfos.map((info: StakingInfo, index) => (
					<FarmCard key={index} stakingInfo={info} stakingAPY={getPoolApy(info?.pair)} isLPFarm={true} />
				))}
			{farmIndex === GlobalConst.farmIndex.DUALFARM_INDEX &&
				stakingDualInfos &&
				stakingDualInfos.map((info: DualStakingInfo, index) => (
					<FarmCard key={index} stakingInfo={info} stakingAPY={getPoolApy(info?.pair)} />
				))}
			<div ref={loadMoreRef} />
		</>
	);
