			...
			if (pairId && transactions && transactions.length > 0) {
				...
				if (txns) {
					const filteredTxns = txns.filter(
						(txn) => !transactions.filter((tx) => tx.transaction.id === txn.transaction.id).length,
					);
	...
	return (
		<Box width='100%' mb={3} id='swap-page'>
			...
			{!isProMode ? (
				...
				<Box className='border-top border-bottom bg-palette flex flex-wrap' minHeight='calc(100vh - 140px)'>
					<Box width={isMobile ? 1 : '450px'} padding='20px 0' className={isMobile ? '' : 'border-right'}>
						<Box className='flex justify-between items-center' padding='0 24px' mb={3}>
							<h4>{t('swap')}</h4>
					...
					{infoPos === 'left' && (
						...
						>
							<SwapProInfo token1={token1} token2={token2} transactions={transactions} />
						</Box>
					)}
					...
					{infoPos === 'right' && (
						<Box className={isMobile ? 'border-top' : 'border-left'} width={isTablet ? 1 : 250}>
							<SwapProInfo token1={token1} token2={token2} transactions={transactions} />
						</Box>
					)}
				</Box>
