...
import { Token } from '@smartdev1990/quick-sdk1';
...
const SwapProChartTrade: React.FC<{
	...
	transactions?: any[];
}> = ({ showChart, showTrades, token1, token2, pairAddress, pairTokenReversed, transactions }) => {
	...
	const TradesTable = () => (
		<TableVirtuoso
			...
			itemContent={(index, tx) => {
				...
				const txAmount0 = Number(tx.amount0In) > 0 ? tx.amount0In : tx.amount0Out;
				const txAmount1 = Number(tx.amount1In) > 0 ? tx.amount1In : tx.amount1Out;
				const token1Amount =
					tx.pair.token0.id.toLowerCase() === token1.address.toLowerCase() ? txAmount0 : txAmount1;
				const token2Amount =
					tx.pair.token0.id.toLowerCase() === token1.address.toLowerCase() ? txAmount1 : txAmount0;
				const txPrice = Number(tx.amountUSD) / token1Amount;
				return (
					<>
						<td align='left'>{dayjs.unix(tx.transaction.timestamp).format('MMM DD, hh:mm:ss a')}</td>
						...
						<td className={txType} align='right'>
							{chainId ? (
								<a
									href={getEtherscanLink(chainId, tx.transaction.id, 'transaction')}
									target='_blank'
									rel='noreferrer'
								>
	...
	return (
		<ReflexContainer orientation='horizontal'>
			...
			{showChart && showTrades && (
				<ReflexSplitter>
					<Box width={1} height='2px' className='flex justify-center items-center'>
						<Height />
					</Box>
				</ReflexSplitter>
			)}
