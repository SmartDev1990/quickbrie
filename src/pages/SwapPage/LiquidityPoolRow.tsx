...
const LiquidityPoolRow: React.FC<{
	...
	const dayVolumeUSD =
		Number(pair.oneDayVolumeUSD ? pair.oneDayVolumeUSD : pair.oneDayVolumeUntracked) *
	...
	return (
		<Box key={key} className='liquidityContent'>
			<Box className='flex items-center' width={isMobile ? 1 : 0.5}>
				<DoubleCurrencyLogo currency0={token0 ?? undefined} currency1={token1 ?? undefined} size={28} />
				<small style={{ marginLeft: 12 }}>
					{pair.token0.symbol.toUpperCase()} / {pair.token1.symbol.toUpperCase()}
				</small>
			</Box>
			<Box width={isMobile ? 1 : 0.2} mt={isMobile ? 2.5 : 0} className='flex justify-between'>
				{isMobile && <small className='text-secondary'>{t('tvl')}</small>}
				<small>${formatCompact(liquidity)}</small>
			</Box>
			<Box width={isMobile ? 1 : 0.15} mt={isMobile ? 1 : 0} className='flex justify-between'>
			...
			>
				...
				<small className={`text-right ${apy < 0 ? 'text-error' : 'text-success'}`}>{apy.toFixed(2)}%</small>
			</Box>
		</Box>
	);
};
