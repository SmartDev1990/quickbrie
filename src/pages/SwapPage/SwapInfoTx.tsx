...
const SwapInfoTx: React.FC<{
	...
	const filteredTxs = transactions?.filter((tx) => tx.transaction.timestamp >= firstTime);
		...
		<>
			...
			<Box className='swapTxInfo'>
				<Box>
					...
					{filteredTxs ? <small>{filteredTxs.length}</small> : <Skeleton width={60} height={14} />}
				</Box>
				<Divider />
				<Box>
					<small className='text-secondary'>{t('buys')}:</small>
					<small>{filteredBuyTxs ? filteredBuyTxs.length : <Skeleton width={60} height={14} />}</small>
				</Box>
				<Divider />
				<Box>
					<small className='text-secondary'>{t('sells')}:</small>
					<small>{filteredSellTxs ? filteredSellTxs.length : <Skeleton width={60} height={14} />}</small>
				</Box>
				<Divider />
				<Box>
