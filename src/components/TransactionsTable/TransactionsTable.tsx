...
const TransactionsTable: React.FC<TransactionsTableProps> = ({ data }) => {
	...
	const txHeadCells = [
		{
			...
			label: (
				<Box className='flex items-center'>
					<small className={txFilter === -1 ? '' : 'text-secondary'} onClick={() => setTxFilter(-1)}>
						{t('all')}
					</small>
					<small
	...
	const desktopHTML = (txn: any) => {
		return [
			...
			{
				html: <p>{dayjs(Number(txn.transaction.timestamp) * 1000).fromNow()}</p>,
			},
		];
	};
			...
			{isMobile && (
				<Box className='txTableFilterMobile'>
					...
					<Box onClick={() => setTxFilter(TxnType.SWAP)}>
						<p className={txFilter === TxnType.SWAP ? '' : 'text-secondary'}>{t('swap')}</p>
					</Box>
					<Box onClick={() => setTxFilter(TxnType.ADD)}>
						<p className={txFilter === TxnType.ADD ? '' : 'text-secondary'}>{t('add')}</p>
					</Box>
					<Box onClick={() => setTxFilter(TxnType.REMOVE)}>
						<p className={txFilter === TxnType.REMOVE ? '' : 'text-secondary'}>{t('remove')}</p>
					</Box>
				</Box>
			)}
			<CustomTable
				...
				rowsPerPage={10}
				data={data.filter((item) => (txFilter === -1 ? true : item.type === txFilter))}
