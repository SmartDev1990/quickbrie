...
const TransactionPopup: React.FC<TransactionPopupProps> = ({ hash, pending, success, summary }) => {
	...
	return (
		<>
			<Box mb={1.5} className='flex justify-between items-center'>
				<small className={`weight-600 ${pending ? 'text-yellow3' : success ? 'text-success' : 'text-error'}`}>
			...
			<small>{summary ?? 'Hash: ' + hash.slice(0, 8) + '...' + hash.slice(58, 65)}</small>
			{pending && (
