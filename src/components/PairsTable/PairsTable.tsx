...
const headCells = () => [
	...
	},
	{
		...
		numeric: false,
		label: 'Liquidity',
		sortKey: (pair: any) => (pair.trackedReserveUSD ? pair.trackedReserveUSD : pair.reserveUSD),
	},
	{
		id: 'pairdayVolume',
		numeric: false,
		label: '24h Volume',
		sortKey: (pair: any) => (pair.oneDayVolumeUSD ? pair.oneDayVolumeUSD : pair.oneDayVolumeUntracked),
	},
	{
		id: 'pairweekVolume',
		numeric: false,
		label: '7d Volume',
		sortKey: (pair: any) => (pair.oneWeekVolumeUSD ? pair.oneWeekVolumeUSD : pair.oneWeekVolumeUntracked),
	},
	{
		id: 'pairdayFee',
		numeric: false,
...
const PairTable: React.FC<PairsTableProps> = ({ data, showPagination = true }) => {
	...
	const mobileHTML = (pair: any, index: number) => {
		...
		const oneDayVolume = pair.oneDayVolumeUSD ? pair.oneDayVolumeUSD : pair.oneDayVolumeUntracked;
		const oneWeekVolume = pair.oneWeekVolumeUSD ? pair.oneWeekVolumeUSD : pair.oneWeekVolumeUntracked;
		const oneDayFee = formatNumber(Number(oneDayVolume) * GlobalConst.utils.FEEPERCENT);
		return (
			<Box mt={index === 0 ? 0 : 3}>
				<Box className='flex items-center' mb={1}>
					...
					>
						{bookmarkPairs.indexOf(pair.id) > -1 ? <StarChecked /> : <StarUnchecked />}
					</Box>
