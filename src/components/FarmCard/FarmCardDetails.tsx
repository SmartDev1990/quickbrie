...
import { TokenAmount, Pair } from '@smartdev1990/quick-sdk1';
...
const FarmCardDetails: React.FC<{
	...
	const { parsedAmount } = useDerivedStakeInfo(stakeAmount, stakingInfo.stakedAmount?.token, userLiquidityUnstaked);
	...
	const pairContract = usePairContract(
		stakingInfo && stakingInfo.lp && stakingInfo.lp !== '' ? stakingInfo.lp : dummyPair?.liquidityToken.address,
	);
	...
	const tvl = getTVLStaking(stakedAmounts?.totalStakedUSD, stakedAmounts?.totalStakedBase);
	...
	const claimEnabled =
		!attemptClaiming &&
		(isLPFarm
			? lpStakingInfo.earnedAmount && lpStakingInfo.earnedAmount.greaterThan('0')
			: dualStakingInfo.earnedAmountA && dualStakingInfo.earnedAmountA.greaterThan('0'));

	return (
		<>
			<Box className={`farmCardDetails ${stakingInfo?.ended ? 'justify-end' : 'justify-between'}`}>
				{stakingInfo && (
					<>
						{isMobile && (
							<>
								...
								<Box className='farmCardMobileRow'>
									...
									<Box textAlign='right'>
										<small>
											${(isLPFarm ? lpRewards : dualRewards).toLocaleString()} /{t('day')}
										</small>
										<br />
										{isLPFarm ? (
							...
							<Box className='buttonWrapper'>
								<Box className='flex justify-between'>
									...
									<Box className='flex flex-col items-end'>
										...
										>
											{t('get')} {currency0?.symbol} / {currency1?.symbol} {t('lp')}
										</Link>
									</Box>
								</Box>
								<Box className='inputVal' mb={2} mt={2} p={2}>
									...
									<small
										className={
											userLiquidityUnstaked && userLiquidityUnstaked.greaterThan('0')
												? 'text-primary'
												: 'text-hint'
										}
										onClick={() => {
						...
						<Box className='buttonWrapper' mx={isMobile ? 0 : 2} my={2}>
							...
							>
								<p>{attemptUnstaking ? t('unstakingLPTokens') : t('unstakeLPTokens')}</p>
							</Box>
						</Box>
						<Box className='buttonWrapper'>
								...
								{isLPFarm ? (
									...
									<>
										<Box mb={1} display='flex'>
											<CurrencyLogo currency={unwrappedToken(dualStakingInfo.rewardTokenA)} />
											<CurrencyLogo currency={unwrappedToken(dualStakingInfo.rewardTokenB)} />
										</Box>
										<Box mb={1} textAlign='center'>
											<p>{earnedUSDStr}</p>
											<p className='text-secondary'>
			...
			{mainRewardRate?.greaterThan('0') && (
				<Box className='dailyRateWrapper'>
					<Box>
						...
						<small>
							{formatMulDivTokenAmount(mainRewardRate, GlobalConst.utils.ONEDAYSECONDS)}{' '}
							{isLPFarm ? lpStakingInfo.rewardToken.symbol : dualStakingInfo.rewardTokenA.symbol} /{' '}
							{t('day')}
						</small>
					</Box>
					{!isLPFarm && (
						<Box>
							...
							<small>
								{formatMulDivTokenAmount(dualStakingInfo.rewardRateB, GlobalConst.utils.ONEDAYSECONDS)}{' '}
