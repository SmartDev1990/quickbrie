...
export interface CustomTableProps<T> {
	...
	desktopHTML: (item: any, index: number, page: number, rowsPerPage: number) => any;
}
			...
			{mobileWindowSize ? (
				...
				<DataTable
					...
					renderRow={(item, index, page, rowsPerPage) => {
						return (
							<TableRow key={index}>
								{desktopHTML(item, index, page, rowsPerPage).map((cellItem: any, ind: number) => (
									<TableCell key={ind} className={cellItem.button ? 'buttonCell' : ''}>
										{cellItem.html}
									</TableCell>
								))}
							</TableRow>
