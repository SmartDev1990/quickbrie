...
import { TokenAmount } from '@smartdev1990/quick-sdk1';
...
const StakeSyrupModal: React.FC<StakeSyrupModalProps> = ({ open, onClose, syrup }) => {
	...
	const { parsedAmount, error } = useDerivedSyrupInfo(typedValue, syrup.stakedAmount?.token, userLiquidityUnstaked);
	...
	return (
		<CustomModal open={open} onClose={onClose}>
			<Box paddingX={3} paddingY={4}>
				...
				<Box mt={3} className='bg-default border-gray14' borderRadius='10px' padding='16px'>
					...
					<Box mt={2} className='flex items-center'>
						<NumericalInput
							...
							onUserInput={(value) => {
								...
								const exactTypedValue = getValueTokenDecimals(value, syrup.stakedAmount?.token);
								// this is to avoid input amount more than balance
								if (Number(exactTypedValue) <= totalBalance) {
									setTypedValue(exactTypedValue);
									setStakePercent(
										totalBalance > 0 ? (Number(exactTypedValue) / totalBalance) * 100 : 0,
									);
								}
							}}
						/>
				...
				<Box mt={2} className='flex items-center justify-between'>
					...
					<p>
						{hypotheticalRewardRate
							? formatNumber(Number(hypotheticalRewardRate.toExact()) * getSecondsOneDay())
							: '-'}{' '}
