...
const StakeQuickModal: React.FC<StakeQuickModalProps> = ({ open, onClose }) => {
	...
	const userLiquidityUnstaked = useTokenBalance(account ?? undefined, quickToken);

	const [typedValue, setTypedValue] = useState('');
	const [stakePercent, setStakePercent] = useState(0);
	const [approving, setApproving] = useState(false);
	const { parsedAmount, error } = useDerivedLairInfo(typedValue, quickToken, userLiquidityUnstaked);
							...
							<ColoredSlider
								...
								handleChange={(evt: any, value) => {
									...
									setTypedValue(
										quickBalance
											? stakePercent < 100
												? ((Number(quickBalance.toExact()) * stakePercent) / 100).toString()
