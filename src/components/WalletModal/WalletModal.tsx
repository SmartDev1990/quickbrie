...
import { injected, portis, safeApp } from 'connectors';
...
const WalletModal: React.FC<WalletModalProps> = ({ pendingTransactions, confirmedTransactions, ENSName }) => {
	...
	}, [account, previousAccount, toggleWalletModal, walletModalOpen, deactivate, error]);
			...
			return (
				...
				!option.mobileOnly && (
					<Option
						...
						active={
							option.connector === connector &&
							(connector !== injected || isMetamask === (option.name === GlobalConst.walletName.METAMASK))
						}
						color={option.color}
						link={option.href}
						header={option.name}
			...
			return (
				<Box position='relative'>
					...
					<Box mt={2} textAlign='center'>
						<h6>{error instanceof UnsupportedChainIdError ? t('wrongNetwork') : t('errorConnect')}</h6>
					</Box>
