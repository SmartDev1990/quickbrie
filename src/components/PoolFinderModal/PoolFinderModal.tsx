...
import { Currency, TokenAmount, ETHER, JSBI } from '@smartdev1990/quick-sdk1';
import { ArrowLeft, Plus } from 'react-feather';
import { Box } from '@material-ui/core';
import { CustomModal, CurrencyLogo, CurrencySearchModal, MinimalPositionCard } from 'components';
			...
			<Box paddingX={3} paddingY={4}>
				...
				<Box className='poolFinderInfo border'>
					{currency0 && currency1 ? (
						pairState === PairState.EXISTS ? (
							hasPosition && pair ? (
								...
								<Box textAlign='center'>
									...
									<Link
										to={`/pools?currency0=${currencyId(currency0)}&currency1=${currencyId(
											currency1,
										)}`}
							...
							<Box textAlign='center'>
								<p>{t('nopoolFound')}.</p>
								<Link
									to={`/pools?currency0=${currencyId(currency0)}&currency1=${currencyId(currency1)}`}
									className='text-primary no-decoration'
									onClick={onClose}
								>
									{t('createPool')}.
						...
						<p>{!account ? t('connectWalletToFindPool') : t('selectTokenFindLiquidity')}</p>
					)}
				</Box>
			</Box>
			{showSearch && (
