...
import { Currency, ETHER, JSBI, Percent } from '@smartdev1990/quick-sdk1';
			...
			<Box>
				...
				<Box mb={6} textAlign='center'>
					<p className='weight-600'>
						...
						<br />
						{t('youwillreceive')} {formatTokenAmount(parsedAmounts[Field.CURRENCY_A])} {currency0.symbol}{' '}
						{t('and')} {formatTokenAmount(parsedAmounts[Field.CURRENCY_B])} {currency1.symbol}
					</p>
				</Box>
		...
		<CustomModal open={open} onClose={onClose}>
			<Box paddingX={3} paddingY={4}>
				...
				<Box className='flex items-center justify-between'>
					<ArrowLeft className='text-secondary cursor-pointer' onClick={onClose} />
					...
					<Box className='flex items-center'>
						<Box flex={1} mr={2} mt={0.5}>
							<ColoredSlider
								...
								handleChange={(event, value) => setInnerLiquidityPercentage(value as number)}
							/>
						</Box>
				...
				<Box className='removeLiquidityInfo bg-secondary1'>
					...
					<Box>
						<p>{t('yourPoolShare')}</p>
						<p>{poolTokenPercentage ? poolTokenPercentage.toSignificant() + '%' : '-'}</p>
					</Box>
				</Box>
				{pair && (
					<Box className='flex justify-between items-center' mt={2} px={2}>
						<small>
							1 {currency0.symbol} = {tokenA ? pair.priceOf(tokenA).toSignificant(6) : '-'}{' '}
							{currency1.symbol}
						</small>
						<small>
							1 {currency1.symbol} = {tokenB ? pair.priceOf(tokenB).toSignificant(6) : '-'}{' '}
							{currency0.symbol}
						</small>
					</Box>
				)}
