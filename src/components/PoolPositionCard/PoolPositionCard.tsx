...
import { ChainId, Pair } from '@smartdev1990/quick-sdk1';
...
const PoolPositionCard: React.FC<{ pair: Pair }> = ({ pair }) => {
	...
	const stakingInfo = useMemo(
		() => (stakingInfos.length > 0 ? stakingInfos[0] : dualStakingInfos.length > 0 ? dualStakingInfos[0] : null),
	...
	const apyWithFee = useMemo(() => {
		if (stakingInfo && bulkPairData) {
			...
			return formatAPY(getAPYWithFee(stakingInfo.perMonthReturnInRewards ?? 0, oneYearFee));
		}
	}, [stakingInfo, bulkPairData]);

	return (
		<Box className={`poolPositionCard ${showMore ? 'bg-secondary2' : 'bg-transparent'}`}>
			<Box className='poolPositionCardTop'>
				...
				<Box className='flex items-center text-primary cursor-pointer' onClick={() => setShowMore(!showMore)}>
