...
import { Pair, JSBI, Percent } from '@smartdev1990/quick-sdk1';
		...
		<>
			<Box className='poolPositionCardDetails'>
				...
				<Box className='cardRow'>
					...
					<small>{poolTokenPercentage ? poolTokenPercentage.toSignificant() + '%' : '-'}</small>
				</Box>
				...
				<Box className='poolButtonRow'>
					...
					<Button
						...
						onClick={() => {
							history.push(
								`/pools?currency0=${currencyId(currency0)}&currency1=${currencyId(currency1)}`,
							);
						}}
					>
						<small>{t('add')}</small>
