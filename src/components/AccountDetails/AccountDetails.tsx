...
const AccountDetails: React.FC<AccountDetailsProps> = ({
	...
	function formatConnectorName() {
		const name = getWalletKeys(connector).map((k) => SUPPORTED_WALLETS[k].name)[0];
		return (
			<small>
	...
	return (
		<Box paddingX={3} paddingY={4}>
				...
				<Box className='flex justify-between items-center'>
					...
					{chainId && account && (
						<a
							...
							href={chainId && getEtherscanLink(chainId, ENSName ? ENSName : account, 'address')}
							target='_blank'
			...
			{!!pendingTransactions.length || !!confirmedTransactions.length ? (
				<>
					<Box className='flex justify-between items-center' px={2} pt={2} mb={1}>
						<small>{t('recentTransactions')}</small>
						<small className='cursor-pointer' onClick={clearAllTransactionsCallback}>
							{t('clearAll')}
						</small>
					</Box>
