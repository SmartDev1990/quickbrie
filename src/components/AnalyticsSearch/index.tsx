...
import { GlobalConst, MAINNET_CHAIN } from 'constants/index';
					...
					.filter((pair) => {
						...
						const regexMatches = Object.keys(pair).map((field) => {
							...
							if (field === 'token0' || field === 'token1') {
								return (
									pair[field].symbol.match(new RegExp(escapeRegExp(searchVal), 'i')) ||
				...
				const foundTokensWithData = await Promise.all(
					allTokens.map(async (token: any) => {
						...
						if (
							...
							const oneDayVolumeUSD = (token?.tradeVolumeUSD ?? 0) - (oneDayHistory?.tradeVolumeUSD ?? 0);
			...
			{menuOpen && (
				<div ref={wrapperRef} className='searchWidgetContent'>
					...
					{filteredPairs.slice(0, pairsShown).map((val, ind) => {
						const currency0 = new Token(MAINNET_CHAIN, getAddress(val.token0.id), val.token0.decimals);
						const currency1 = new Token(MAINNET_CHAIN, getAddress(val.token1.id), val.token1.decimals);
						return (
							<Box
								key={ind}
					...
					<Box className='searchWidgetShowMore' onClick={() => setPairsShown(pairsShown + 5)}>
						<small>{t('showMore')}</small>
					</Box>
					<h6>{t('tokens')}</h6>
					{filteredTokens.slice(0, tokensShown).map((val, ind) => {
						const currency = new Token(MAINNET_CHAIN, getAddress(val.id), val.decimals);
						return (
							<Box
								key={ind}
								className='searchWidgetRow'
					...
					<Box className='searchWidgetShowMore' onClick={() => setTokensShown(tokensShown + 5)}>
						<small>{t('showMore')}</small>
					</Box>
				</div>
			)}
