...
export interface DataTableProps<T> {
	...
	data: T[];
	renderRow: (item: T, index: number, page: number, rowsPerPage: number) => React.ReactNode;
...
const DataTable: React.FC<DataTableProps<any>> = ({
	...
	const handleRequestSort = (event: React.MouseEvent<unknown>, property: HeadCell<any>) => {
										...
										>
											<Box
												className={`headCellLabel${
													orderBy.id === headCell.id ? ' sortRequestedHeadLabel' : ''
												}`}
											>
												{headCell.label}
											</Box>
