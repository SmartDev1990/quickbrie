...
const Header: React.FC = () => {
	...
	return (
		<Box className='header'>
			...
			/>
			<Link to='/'>
				<img src={mobileWindowSize ? QuickIcon : QuickLogo} alt='QuickLogo' height={60} />
			</Link>
			{!tabletWindowSize && (
				<Box className='mainMenu'>
					{menuItems.map((val, index) => (
						<Link
							...
							key={index}
							id={val.id}
							className={`menuItem ${pathname.indexOf(val.link) > -1 ? 'active' : ''}`}
						>
							<small>{val.text}</small>
							{val.isNew && (
								<>
									<img src={NewTag} alt='new menu' width={46} />
									<img className='menuItemSparkle menuItemSparkleLeft' src={SparkleLeft} />
									<img className='menuItemSparkle menuItemSparkleRight' src={SparkleRight} />
									<img className='menuItemSparkle menuItemSparkleBottom' src={SparkleBottom} />
									<img className='menuItemSparkle menuItemSparkleTop' src={SparkleTop} />
								</>
							)}
						</Link>
					))}
					...
					<Box className='mobileMenu'>
						{menuItems.slice(0, 4).map((val, index) => (
							<Link
								to={val.link}
								key={index}
								className={pathname.indexOf(val.link) > -1 ? 'active' : 'menuItem'}
							>
								<small>{val.text}</small>
							</Link>
						))}
							...
							{openDetailMenu && (
								<Box className='subMenuWrapper'>
									<Box className='subMenu'>
										...
										{outLinks.map((item, ind) => (
											<a href={item.link} key={ind} onClick={() => setOpenDetailMenu(false)}>
												<small>{item.text}</small>
											</a>
										))}
									</Box>
			...
			<Box>
				...
				{account && (!ethereum || isSupportedNetwork(ethereum)) ? (
					<Box id='web3-status-connected' className='accountDetails' onClick={toggleWalletModal}>
					...
					<Box
						className={`connectButton ${
							ethereum && !isSupportedNetwork(ethereum) ? 'bg-error' : 'bg-primary'
						}`}
						onClick={() => {
					...
					>
						{ethereum && !isSupportedNetwork(ethereum) ? t('wrongNetwork') : t('connectWallet')}
