		...
		<Box padding={4}>
			...
			<Box className='flex justify-between' mt={2}>
				{chainId && hash && (
					...
					>
						<Button className='txSubmitButton'>{t('viewonBlockExplorer')}</Button>
					</a>
				)}
				<Button className='txSubmitButton' style={{ width: '48%' }} onClick={onDismiss}>
					{t('close')}
				</Button>
			</Box>
		</Box>
...
export const ConfirmationModalContent: React.FC<ConfirmationModalContentProps> = ({ title, onDismiss, content }) => {
	return (
		<Box padding={4}>
			<Box className='txModalHeader'>
				<h5>{title}</h5>
