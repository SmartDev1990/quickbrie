...
import { Currency, Token, ETHER, TokenAmount } from '@smartdev1990/quick-sdk1';
...
const AddLiquidity: React.FC<{
	...
	const [addLiquidityErrorMessage, setAddLiquidityErrorMessage] = useState<string | null>(null);
	...
	const onAddLiquidity = async () => {
		...
		if (currencies[Field.CURRENCY_A] === ETHER || currencies[Field.CURRENCY_B] === ETHER) {
			...
			args = [
				wrappedCurrency(tokenBIsETH ? currencies[Field.CURRENCY_A] : currencies[Field.CURRENCY_B], chainId)
					?.address ?? '', // token
		...
		<Box>
			...
			{currencies[Field.CURRENCY_A] && currencies[Field.CURRENCY_B] && pairState !== PairState.INVALID && price && (
				<Box my={2}>
					<Box className='swapPrice'>
						<small>
							1 {currencies[Field.CURRENCY_A]?.symbol} = {price.toSignificant(3)}{' '}
							{currencies[Field.CURRENCY_B]?.symbol}{' '}
						</small>
						<small>
							1 {currencies[Field.CURRENCY_B]?.symbol} = {price.invert().toSignificant(3)}{' '}
							{currencies[Field.CURRENCY_A]?.symbol}{' '}
						</small>
					</Box>
					<Box className='swapPrice'>
						<small>{t('yourPoolShare')}:</small>
						<small>{poolTokenPercentage ? poolTokenPercentage.toSignificant(6) + '%' : '-'}</small>
					</Box>
					<Box className='swapPrice'>
			...
			<Box className='swapButtonWrapper flex-wrap'>
				...
				<Button
					...
					disabled={
						Boolean(account) &&
						(Boolean(error) || approvalA !== ApprovalState.APPROVED || approvalB !== ApprovalState.APPROVED)
					}
