...
import { Currency, CurrencyAmount, JSBI, Token, Trade } from '@smartdev1990/quick-sdk1';
...
const Swap: React.FC<{
	...
	const [
		{ showConfirm, txPending, tradeToConfirm, swapErrorMessage, attemptingTxn, txHash },
		setSwapState,
	] = useState<{
		showConfirm: boolean;
		txPending?: boolean;
		...
		<Box>
			...
			{trade && trade.executionPrice && (
				<Box className='swapPrice'>
					<small>{t('price')}:</small>
					<small>
						1 {(mainPrice ? currencies[Field.INPUT] : currencies[Field.OUTPUT])?.symbol} ={' '}
						{(mainPrice ? trade.executionPrice : trade.executionPrice.invert()).toSignificant(6)}{' '}
						{(mainPrice ? currencies[Field.OUTPUT] : currencies[Field.INPUT])?.symbol}{' '}
						<PriceExchangeIcon
							onClick={() => {
			...
			{!showWrap && isExpertMode && (
				<Box className='recipientInput'>
					<Box className='recipientInputHeader'>
						{recipient !== null ? <ArrowDown size='16' color='white' /> : <Box />}
						<Button onClick={() => onChangeRecipient(recipient !== null ? null : '')}>
							{recipient !== null ? `- ${t('removeSend')}` : `+ ${t('addSendOptional')}`}
						</Button>
					</Box>
					{recipient !== null && (
						<AddressInput
