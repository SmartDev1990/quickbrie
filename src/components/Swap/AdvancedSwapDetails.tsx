	...
	return (
		<Box mt={1.5}>
			...
			<Box className='summaryRow'>
				...
				<Box onClick={() => setOpenSettingsModal(true)} className='swapSlippage'>
			...
			<Box className='summaryRow'>
				...
				<Box>
					<small>
						{formatTokenAmount(slippageAdjustedAmounts[isExactIn ? Field.OUTPUT : Field.INPUT])}{' '}
...
export const AdvancedSwapDetails: React.FC<AdvancedSwapDetailsProps> = ({ trade }) => {
	...
	return <>{trade && <TradeSummary trade={trade} allowedSlippage={allowedSlippage} />}</>;
};
