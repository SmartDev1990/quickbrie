...
const SettingsModal: React.FC<SettingsModalProps> = ({ open, onClose }) => {
	...
	const slippageInputIsValid =
		slippageInput === '' ||
		(userSlippageTolerance / 100).toFixed(2) === Number.parseFloat(slippageInput).toFixed(2);
			...
			<CustomModal open={expertConfirm} onClose={() => setExpertConfirm(false)}>
				<Box paddingX={3} paddingY={4}>
					<Box mb={3} className='flex justify-between items-center'>
						<h5>{t('areyousure')}</h5>
						<CloseIcon className='cursor-pointer' onClick={() => setExpertConfirm(false)} />
					</Box>
					<Divider />
					<Box mt={2.5} mb={1.5}>
					...
					<Box className='expertConfirmInput'>
						<input value={expertConfirmText} onChange={(e: any) => setExpertConfirmText(e.target.value)} />
					</Box>
					<Box
