import { CurrencyAmount, ETHER, Token } from '@smartdev1990/quick-sdk1';
...
const CurrencyRow: React.FC<CurrenyRowProps> = ({
	...
	const isMetamask = ethereum && ethereum.isMetaMask && Number(ethereum.chainId) === 32520 && isOnSelectedList;
			...
			<Box className='currencyRow'>
				...
				<Box ml={1} height={32}>
					...
					{isOnSelectedList ? (
						...
						<Box className='flex items-center'>
							<span>{customAdded ? t('addedByUser') : t('foundByAddress')}</span>
							<Box
								ml={0.5}
					...
					{balance ? (
						<>
							...
							<span className='text-secondary'>
								$
								{(
									Number(balance.toExact()) * (usdPrice ? Number(usdPrice.toSignificant()) : 0)
								).toLocaleString()}
							</span>
						</>
					) : account ? (
