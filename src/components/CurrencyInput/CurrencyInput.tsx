...
import { Currency } from '@smartdev1990/quick-sdk1';
...
const CurrencyInput: React.FC<CurrencyInputProps> = ({
	...
	const selectedCurrencyBalance = useCurrencyBalance(account ?? undefined, currency);
	...
	return (
		<Box id={id} className={`swapBox${showPrice ? ' priceShowBox' : ''} ${bgClass ?? 'bg-secondary2'}`}>
			...
			<Box className='flex justify-between'>
				...
				<small className='text-secondary'>${(usdPrice * Number(amount)).toLocaleString()}</small>
			</Box>
			{modalOpen && (
				<CurrencySearchModal
					isOpen={modalOpen}
