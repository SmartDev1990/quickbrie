...
const SyrupCard: React.FC<{ syrup: SyrupInfo; dQUICKAPY: string }> = ({ syrup, dQUICKAPY }) => {
					...
					<>
								...
								<Box mt={0.25}>
									<span>
										...
										{syrup.rewardTokenPriceinUSD
											? (syrup.rate * syrup.rewardTokenPriceinUSD).toLocaleString()
											: '-'}{' '}
										<span className='text-secondary'>/ {t('day')}</span>
									</span>
								</Box>
						...
						<Box width={0.2} textAlign='right'>
							<Box className='flex items-center justify-end' mb={0.25}>
								...
								<small style={{ marginLeft: 5 }}>{formatTokenAmount(syrup.earnedAmount)}</small>
							</Box>
							<small className='text-secondary'>{getEarnedUSDSyrup(syrup)}</small>
						</Box>
					</>
				)}
			</Box>
			{expanded && syrup && <SyrupCardDetails syrup={syrup} dQUICKAPY={dQUICKAPY} />}
		</Box>
	);
};
