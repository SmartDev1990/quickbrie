...
const SyrupCardDetails: React.FC<{ syrup: SyrupInfo; dQUICKAPY: string }> = ({ syrup, dQUICKAPY }) => {
	...
	const userLiquidityUnstaked = useTokenBalance(account ?? undefined, syrup.stakedAmount?.token);
	...
	const depositAmount =
		syrup && syrup.valueOfTotalStakedAmountInUSDC
			...
			: `${formatTokenAmount(syrup?.totalStakedAmount)} ${syrup?.stakingToken.symbol}`;
	...
	return (
		<>
			{openStakeModal && syrup && (
				<StakeSyrupModal open={openStakeModal} onClose={() => setOpenStakeModal(false)} syrup={syrup} />
			)}
			{syrup && (
				<>
					<Divider />
					<Box padding={isMobile ? 2 : 3}>
						...
						<Box className='flex items-center justify-between' mb={1.5}>
							...
							<small>
								{userLiquidityUnstaked ? formatTokenAmount(userLiquidityUnstaked) : 0}{' '}
								{syrup.stakingToken.symbol}
								<small className='text-secondary' style={{ marginLeft: 4 }}>
									$
									{userLiquidityUnstaked
										? (stakingTokenPrice * Number(userLiquidityUnstaked.toExact())).toLocaleString()
										: 0}
								</small>
							</small>
						</Box>
						<Box className='flex items-center justify-between' mb={1.5}>
							...
							<small>
								{formatTokenAmount(syrup.stakedAmount)} {syrup.stakingToken.symbol}
								<small className='text-secondary' style={{ marginLeft: 4 }}>
									{syrup.stakedAmount
										? `$${(
						...
						<Box className='flex flex-wrap items-center justify-between'>
							{!isMobile && <SyrupTimerLabel exactEnd={exactEnd} isEnded={syrup?.ended} />}
							{isMobile ? (
								<>
									...
									>
										...
										{syrup.stakedAmount && syrup.stakedAmount.greaterThan('0') && <UnstakeButton />}
									</Box>
								</>
							) : (
						...
						{syrup.rewardRate?.greaterThan('0') && (
							<Box className='dailyRateWrapper'>
								<Box>
									...
									<small>
										{formatMulDivTokenAmount(syrup.rewardRate, GlobalConst.utils.ONEDAYSECONDS)}{' '}
