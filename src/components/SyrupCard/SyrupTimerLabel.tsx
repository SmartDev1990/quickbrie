...
const SyrupTimerLabel: React.FC<{ exactEnd: number; isEnded: boolean }> = ({ exactEnd, isEnded }) => {
	...
	return (
		<>
			{!isEnded && Number.isFinite(timeRemaining) && (
				<Box className={isMobile ? 'flex flex-wrap items-center justify-between' : ''}>
					<p className={`text-secondary ${isMobile ? 'small' : 'caption'}`}>{t('timeRemaining')}</p>
