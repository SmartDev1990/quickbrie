...
const TokensTable: React.FC<TokensTableProps> = ({ data, showPagination = true }) => {
		...
		return (
			<Box mt={index === 0 ? 0 : 3}>
				<Box className='flex items-center' mb={1}>
					...
					>
						{bookmarkTokens.indexOf(token.id) > -1 ? <StarChecked /> : <StarUnchecked />}
					</Box>
					<Link className='no-decoration' to={`/analytics/token/${tokenCurrency.address}`}>
