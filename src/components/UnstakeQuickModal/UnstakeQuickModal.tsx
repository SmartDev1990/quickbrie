...
const UnstakeQuickModal: React.FC<UnstakeQuickModalProps> = ({ open, onClose, isNew }) => {
	...
	return (
		<CustomModal open={open} onClose={onClose}>
			<Box paddingX={3} paddingY={4}>
							...
							<ColoredSlider
								...
								handleChange={(evt, value) => {
									...
									setTypedValue(
										dQuickBalance
											? stakePercent < 100
												? ((Number(dQuickBalance.toExact()) * stakePercent) / 100).toString()
				...
				<Box mt={3}>
					<Button className='stakeButton' disabled={!!error || attempting} onClick={onWithdraw}>
